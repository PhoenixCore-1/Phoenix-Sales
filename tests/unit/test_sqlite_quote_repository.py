...
def make_quote(tenant="tenant-1", customer="customer-1", opportunity=None, quote_number="Q-100"):
    q = Quote(tenant, customer, opportunity or uuid4(), quote_number, "ZAR", date(2026, 12, 31), payment_terms="30 days")
...
def test_list_by_customer_and_opportunity_are_scoped():
    repo = SQLiteQuoteRepository(sqlite3.connect(":memory:"))
    opportunity = uuid4()
    repo.save(make_quote(customer="customer-1", opportunity=opportunity, quote_number="Q-100"))
    repo.save(make_quote(customer="customer-2", opportunity=uuid4(), quote_number="Q-101"))
