...
from phoenix_sales.integrations.fulfilment import FulfilmentLineRequest, FulfilmentRequest, FulfilmentResult, build_fulfilment_request
...
class FulfilmentOrchestrationService:
    ...
    def __init__(self, context: RequestContext, repository: SalesOrderRepository, inventory: InventoryFulfilmentPort) -> None:
        self._context = context
        self._repository = repository
        self._inventory = inventory
        self._processed_correlations: set[str] = set()

    def request_fulfilment(self, order_id: UUID, *, correlation_id: str | None = None, priority: str | None = None, requested_delivery_date: date | None = None, delivery_site: str | None = None, delivery_address: str | None = None) -> SalesOrder:
        self._require_permission()
        order = self._get(order_id)
        if order.status not in {SalesOrderStatus.CONFIRMED, SalesOrderStatus.IN_PROCESS, SalesOrderStatus.PARTIALLY_FULFILLED, SalesOrderStatus.BACKORDER}:
        ...
        for index, line in enumerate(order.lines):
            ...
            if remaining <= 0:
                continue
            lines.append(FulfilmentLineRequest(sales_order_line_id=sales_order_line_reference(order.id, index), item_id=line.item_id, ordered_quantity=line.quantity, required_quantity=remaining, unit=line.unit))
        if not lines:
            raise ValueError("sales order has no outstanding fulfilment requirement")
        request = build_fulfilment_request(tenant_id=order.tenant_id, sales_order_id=order.id, order_number=order.order_number, lines=tuple(lines), commercial_branch_id=order.branch_id, requested_delivery_date=requested_delivery_date, delivery_site=delivery_site, delivery_address=delivery_address, priority=priority, correlation_id=correlation, metadata={"source": "phoenix-sales"})
    ...
    def apply_result(self, result: FulfilmentResult) -> SalesOrder:
        ...
        references = {sales_order_line_reference(order.id, index): (index, line) for index, line in enumerate(order.lines)}
        if not {line.sales_order_line_id for line in result.lines}.issubset(references):
        ...
        for result_line in result.lines:
            ...
            updated_lines[index] = SalesOrderLine(item_id=current.item_id, description=current.description, quantity=current.quantity, unit=current.unit, unit_price=current.unit_price, discount_percent=current.discount_percent, ordered_quantity=current.ordered_quantity, allocated_quantity=result_line.allocated_quantity, fulfilled_quantity=result_line.fulfilled_quantity, backorder_quantity=result_line.backorder_quantity)
    ...
    def _derive_status(order: SalesOrder) -> SalesOrderStatus:
        ...
        if all(line.fulfilled_quantity == 0 and line.backorder_quantity >= line.quantity for line in order.lines):
