...
class SQLiteQuoteRepository:
    ...
    def save(self, quote: Quote) -> Quote:
        self._connection.execute("""
            INSERT INTO quotes (id, tenant_id, customer_id, opportunity_id, quote_number, currency,
                ...
                delivery_terms, customer_reference, internal_reference, notes, created_at, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        """, (str(quote.id), quote.tenant_id, quote.customer_id, str(quote.opportunity_id), quote.quote_number,
              ...
              quote.notes, quote.created_at.isoformat(), quote.updated_at.isoformat()))
    ...
    def _from_row(self, row: sqlite3.Row) -> Quote:
        ...
        return Quote(
            ...
            lines=lines, created_at=datetime.fromisoformat(row["created_at"]), updated_at=datetime.fromisoformat(row["updated_at"]),
        )
